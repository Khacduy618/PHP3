...
<div class="card">
    ...
    <div class=" d-flex align-items-center"> {{-- Added card-title and padding --}}
                ...
                <form action="{{ route('admin.users.index') }}" method="GET" class="px-3 py-2">
                    <div class="input-group">
                        ...
                        <select name="status" class="form-select">
                            ...
                            <option value="inactive" {{ request('status') == 'inactive' ? 'selected' : '' }}>Đã xóa
                            </option>
                        </select>
                        @if(request('sort_by'))
    ...
    <div class="card-body">
        <table class="table table-hover align-middle">
            <thead class="table-light">
                <tr>
                    <th>
                        @php $sortIcon = $sortBy == 'id' ? ($sortDir == 'asc' ? 'ti-arrow-up' : 'ti-arrow-down') :
                        'ti-arrows-sort';
                        $queryParams = array_merge(request()->query(), ['sort_by' => 'id', 'sort_dir' => $sortBy ==
                        'id' && $sortDir == 'asc' ? 'desc' : 'asc']);
                        $url = route('admin.users.index', $queryParams);
                        @endphp
                        <a href="{{ $url }}">
                    ...
                    <th>Ảnh đại diện</th>
                    <th>
                        @php $sortIcon = $sortBy == 'name' ? ($sortDir == 'asc' ? 'ti-arrow-up' : 'ti-arrow-down') :
                        'ti-arrows-sort';
                        $queryParams = array_merge(request()->query(), ['sort_by' => 'name', 'sort_dir' => $sortBy ==
                        'name' && $sortDir == 'asc' ? 'desc' : 'asc']);
                        $url = route('admin.users.index', $queryParams);
                        @endphp
                        <a href="{{ $url }}" data-sort-by="name" data-sort-dir="{{ $sortBy == 'name' && $sortDir == 'asc' ? 'desc' : 'asc' }}">
                            Tên <i class="ti {{ $sortIcon }}"></i>
                        </a>
                    </th>
                    <th>
                        @php $sortIcon = $sortBy == 'email' ? ($sortDir == 'asc' ? 'ti-arrow-up' : 'ti-arrow-down') :
                        'ti-arrows-sort';
                        $queryParams = array_merge(request()->query(), ['sort_by' => 'email', 'sort_dir' => $sortBy ==
                        'email' && $sortDir == 'asc' ? 'desc' : 'asc']);
                        $url = route('admin.users.index', $queryParams);
                        @endphp
                        <a href="{{ $url }}" data-sort-by="email" data-sort-dir="{{ $sortBy == 'email' && $sortDir == 'asc' ? 'desc' : 'asc' }}">
                            Email <i class="ti {{ $sortIcon }}"></i>
                        </a>
                    </th>
                    <th>
                        @php $sortIcon = $sortBy == 'role' ? ($sortDir == 'asc' ? 'ti-arrow-up' : 'ti-arrow-down') :
                        'ti-arrows-sort';
                        $queryParams = array_merge(request()->query(), ['sort_by' => 'role', 'sort_dir' => $sortBy ==
                        'role' && $sortDir == 'asc' ? 'desc' : 'asc']);
                        $url = route('admin.users.index', $queryParams);
                        @endphp
                        <a href="{{$url}}" data-sort-by="role" data-sort-dir="{{ $sortBy == 'role' && $sortDir == 'asc' ? 'desc' : 'asc' }}">
                            Vai trò <i class="ti {{ $sortIcon }}"></i>
                        </a>
                    </th>
                    <th>Trạng thái</th> {{-- Add Status Column --}}
                    <th>
                        @php $sortIcon = $sortBy == 'created_at' ? ($sortDir == 'asc' ? 'ti-arrow-up' :
                        'ti-arrow-down') : 'ti-arrows-sort';
                        $queryParams = array_merge(request()->query(), ['sort_by' => 'created_at', 'sort_dir' =>
                        $sortBy == 'created_at' && $sortDir == 'asc' ? 'desc' : 'asc']);
                        $url = route('admin.users.index', $queryParams);
                        @endphp
                        <a href="{{ $url }}" data-sort-by="created_at" data-sort-dir="{{ $sortBy == 'created_at' && $sortDir == 'asc' ? 'desc' : 'asc' }}">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        asset('admin/assets/images/user/avatar-2.jpg') }}" alt="Avatar" class="user-avtar"
...
@push('scripts') {{-- Use a stack for scripts if your layout supports it, otherwise place script tag directly --}}
    <script>
        ...
        const updateSortLinks = () => {
            const currentParams = {!! json_encode(request()->query()) !!};
            document.querySelectorAll('thead th a').forEach(a => {
                const m = a.href.match(/sort_by=([^&]+)&sort_dir=([^&]+)/);
                if (m) {
                    const newParams = {
                        ...currentParams,
                        sort_by: m[1],
                    ...
                    a.href = getUrlWithParams("{{ route('admin.users.index') }}", newParams);
                }
            });
        }
