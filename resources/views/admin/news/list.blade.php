...
<div class="card">
    <div class="card-header">
        <div class="pc-h-item d-none d-md-inline-flex ms-auto p-2">
            <form action="{{ route('admin.news.list') }}" method="GET" class="inline-block">
                <!-- <div class="row g-3 align-items-center">
                    ...
                    <div class="col-md-2 col-lg-3"> -->
                        <select name="deleted" class="form-select">
                            <option value="">-- Lọc theo trạng thái xóa --</option>
    ...
    <div class="card-body">
        <table class="table table-hover align-middle">
            <thead class="table-light">
                <tr>
                    <th>
                        @php $sortIcon = $sortBy == 'id' ? ($sortDir == 'asc' ? 'ti-arrow-up' : 'ti-arrow-down') : 'ti-arrows-sort';
                         $queryParams = array_merge(request()->query(), ['sort_by' => 'id', 'sort_dir' => $sortBy == 'id' && $sortDir == 'asc' ? 'desc' : 'asc', 'search' => request('search'), 'category_id' => request('category_id'), 'status' => request('status'), 'deleted' => request('deleted')]);
                        @endphp
                        <a href="{{ route('admin.news.list', $queryParams) }}">
                            ID <i class="ti {{ $sortIcon }}"></i>
                        </a>
                    </th>
                    <th>Ảnh</th>
                    <th>
                        @php $sortIcon = $sortBy == 'title' ? ($sortDir == 'asc' ? 'ti-arrow-up' : 'ti-arrow-down') : 'ti-arrows-sort';
                         $queryParams = array_merge(request()->query(), ['sort_by' => 'title', 'sort_dir' => $sortBy == 'title' && $sortDir == 'asc' ? 'desc' : 'asc', 'search' => request('search'), 'category_id' => request('category_id'), 'status' => request('status'), 'deleted' => request('deleted')]);
                        @endphp
                        <a href="{{ route('admin.news.list', $queryParams) }}">
                    ...
                    <th>
                        @php $sortIcon = $sortBy == 'created_at' ? ($sortDir == 'asc' ? 'ti-arrow-up' : 'ti-arrow-down') : 'ti-arrows-sort';
                         $queryParams = array_merge(request()->query(), ['sort_by' => 'created_at', 'sort_dir' => $sortBy == 'created_at' && $sortDir == 'asc' ? 'desc' : 'asc', 'search' => request('search'), 'category_id' => request('category_id'), 'status' => request('status'), 'deleted' => request('deleted')]);
                        @endphp
                        <a href="{{ route('admin.news.list', $queryParams) }}">
